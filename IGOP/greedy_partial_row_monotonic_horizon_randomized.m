function [total_cost, total_reward, tour] = greedy_partial_row_monotonic_horizon_randomized( vine_distance, row_distance, reward_map, beginning, ending, budget, horizon, best_of, num_moves, deeper, randomized, monotonic)
%GREEDY_PARTIAL_ROW_MONOTONIC_HORIZON_RANDOMIZED Builds a monotonic orienteering tour on an IG
%
%	Version: 1.0
%	Date: 05/05/2019
%	Author: Thomas Thayer (tthayer@ucmerced.edu)
%
%	This function solves the Irrigation Graph Orienteering Problem (IGOP) using the Greedy Partial Row (GPR) heuristic, while also ensuring the tour is monotonic, thus avoiding unnecessary up/down movement and conserving budget for greater rewards, using a greedy rolling-horizon and randomization.
%	Assumptions:
...
%		monotonic: A boolean deciding whether or not to make the output tour monotonic
%	Outputs:
...
%                         [h, idxes] = sort(temp_row_branches(:, 9), 'descend');
%                         temp_row_branches = temp_row_branches(idxes, :);
...
%                             top_row = temp_row_branches(1, :);
%                             temp_row_branches = temp_row_branches(2:end, :);
%                         end
%                     end
%                     if ~isempty(temp_muted_branches)
%                         [h, idxes] = sort(temp_muted_branches(:, 9), 'descend');
%                         temp_muted_branches = temp_muted_branches(idxes, :);
...
%                             top_muted = temp_muted_branches(1, :);
%                             temp_muted_branches = temp_muted_branches(2:end, :);
%                         end
%                     end
%                     new_branches = [temp_row_branches; temp_muted_branches];
%                     if ~isempty(new_branches)
%                         [h, idxes] = sort(new_branches(:, 9), 'descend');
%                         new_branches = new_branches(idxes, :);
%                     end
%                     new_branches = [top_row; top_muted; new_branches];
%                     if (best_of < inf) && (size(new_branches, 1) > best_of)
%                         %random
%                         if randomized >= 1
%                             if size(new_branches, 1) > randomized*best_of
%                                 new_branches = new_branches(1:(randomized*best_of), :);
%                             end
...
%                         end
%                         %not random
%                         new_branches = new_branches(1:best_of, :);
%                     end
                    %
                    if ~isempty(temp_row_branches)
                        temp_row_branches = temp_row_branches(temp_row_branches(:, 4) > 0, :);
                    ...
                    if size(new_branches, 1) > best_of
                        [vals, idxes] = max_k(new_branches(:, 9), best_of);
                        new_branches = new_branches(idxes, :);
                    end
                    %
                    if isempty(new_branches)
...
%             end
        ...
        this_tour = tour;
        if monotonic
            [total_cost, total_reward, tour] = make_monotonic(vine_distance, row_distance, initial_reward_map, beginning, ending, total_cost, total_reward, tour, 0);
        end
    ...
    reward_map = initial_reward_map;
    if monotonic
        [total_cost, total_reward, tour] = insert_partial_rows_to_monotonic(vine_distance, row_distance, reward_map, budget, tour, total_cost, inf);
    end
    %% Calculate reward
