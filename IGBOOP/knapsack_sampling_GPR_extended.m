function [ total_cost, total_reward, tour, avoidance_map, sampling_reward ] = knapsack_sampling_GPR_extended( vine_distance, row_distance, reward_map, beginning, ending, reward_budget, sampling_budget, sampling_map, avoidance_map )
%KNAPSACK_SAMPLING_GPR_EXTENDED Solves the BOOP with DM on an IG using knapsack for sampling
%
%	Version: 1.0
%	Date: 03/14/2019
%	Author: Thomas Thayer (tthayer@ucmerced.edu)
%
%	This function solves the Irrigation Graph (IG) Dual Maximization (DM) Bi-Objective Orienteering Problem (BOOP) using the Greedy Partial Row (GPR) heuristic and adds sample locations using the knapsack problem, then using the remaining budget to collect more rewards, presented in https://ieeexplore.ieee.org/abstract/document/8842839
%	Assumptions:
...
%		reward_budget: The max allowable movement cost for collection of rewards in the tour
%		sampling_budget: The max allowable movement cost for collection of samples in the tour
    ...
    while unsatisfied
        ...
        for z=1:3
            ...
            elseif z == 2
                ...
                sampling_reward = 0;
                for i=1:size(tour, 1)
                    if temp_sampling_vector(tour(i)) > 0
                        sampling_reward = sampling_reward + temp_sampling_vector(tour(i));
                        temp_sampling_vector(tour(i)) = 0;
                    end
                end
...
%                             temp = [];
%                             for k=1:size(tour, 1)
%                                 this_row = ceil(tour(k) / size_column);
%                                 this_col = mod(tour(k) - 1, size_column) + 1;
%                                 if this_row == i
...
%                             possibility_list = [possibility_list; temp(idx, :)];
                            ...
                            this_row = ceil(tour / size_column);
                            this_col = mod(tour - 1, size_column) + 1;
                ...
                while modified == 1
                    ...
                    if ~isempty(possibility_list)
                        saved_budgets = floor(sampling_budget);
                            ...
                            for j=(i+1):length(opt_choice)
                                ...
                                if possibility_list(i, 1) == possibility_list(j, 1) && (i~=j)
                                    ...
                                    if (opt_choice(i) == 1) && (opt_choice(j) == 1)
                                        ...
                                        if tour(possibility_list(i, 3)) == tour(possibility_list(j, 3))
                                            modified = 1;
                ...
                for x=1:size(possibility_list, 1)
                    if opt_choice(x) == 1
                        ...
                        detour = [];
                        this_row = ceil(tour(k+compensation) / size_column);
                        this_col = mod(tour(k+compensation) - 1, size_column) + 1;
                        if i == this_row
                            if j >= this_col
                                direction_j = 1;
                            elseif j < this_col
                        ...
                        tour = [tour(1:k+compensation); detour; tour(k+compensation+1:end)];
            ...
            elseif z == 3
                %% Utilize remaining budget for rewards
                budget = sampling_budget + reward_budget;
                reward_map = initial_reward_map;
                for i=1:length(tour)
            ...
            %% Find path
            while (sum(sum(cumulative_reward_1)) > 0 || sum(sum(cumulative_reward_2)) > 0) && (z ~= 2)
                if current_side == 1
