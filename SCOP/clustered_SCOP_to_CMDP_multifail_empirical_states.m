function [states, state_transition_table, initial_state_distribution, absorbing_state] = clustered_SCOP_to_CMDP_multifail_empirical_states(vertex_cluster_rewards, vertex_clusters, t_max, num_time_intervals, time_data, num_state_steps, failure_penalties)
%CLUSTERED_SCOP_TO_CMDP_MULTIFAIL_EMPERICAL_STATES Creates a CMDP state_transition_table from a clustered SCOP using empircal time data
%
%	Version: 1.0
%	Date: 12/02/20
%	Author: Thomas Thayer (tthayer@ucmerced.edu)
%
%	This function creates a Constrained Markov Decision Process (CMDP) formulation from a clustered Stochastic Cost Orienteering Problem (SCOP) that has been solved in expectation as an ordinary Orienteering Problem, and uses empircally collected time data to determine the time discretization for each vertex. First used in "A Resolution Adaptive Algorithm for the Stochastic Orienteering Problem with Chance Constraints" by Thayer, Carpin
%	Assumptions:
		...
		if this_vertex < num_vertex
			...
			for j=1:length(next_vertexes)
				...
				if (counter + size(temp2_table, 1) + 2) > size(state_transition_table, 1)
					state_transition_table = [state_transition_table; zeros((num_vertex-1)*num_time_intervals, 6)];
				end
