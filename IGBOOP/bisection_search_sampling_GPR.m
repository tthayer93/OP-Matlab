function [total_cost, total_reward, tour, avoidance_map, sampling_reward, alpha] = bisection_search_sampling_GPR(vine_distance, row_distance, reward_map, beginning, ending, budget, min_samples, sampling_map, type, avoidance_map)
%BISECTION_SEARCH_SAMPLING_GPR Solves the BOOP with sampling OC on an IG
%
%	Version: 1.0
%	Date: 03/07/2019
%	Author: Thomas Thayer (tthayer@ucmerced.edu)
%
%	This function solves the Irrigation Graph (IG) Objective Constraint (OC) Bi-Objective Orienteering Problem (BOOP) using a bisection search on the Greedy Partial Row (GPR) heuristic with minimum sampling constraint, presented in https://ieeexplore.ieee.org/abstract/document/8842839
%	Assumptions:
...
%     elseif type == 4
%         [total_cost, total_reward, tour, avoidance_map, sampling_reward] = kapsack_sampling_GPR_extended(vine_distance, row_distance, reward_map, beginning, ending, 0, budget, sampling_map);
%     end
...
%     end
    ...
    sampling_reward = 0;
    if type == 1
        [this_total_cost, this_total_reward, this_tour, this_avoidance_map, this_sampling_reward] = dual_budget_GPR(vine_distance, row_distance, reward_map, beginning, ending, budget, 0, sampling_map, avoidance_map);
    elseif type == 2
        [this_total_cost, this_total_reward, this_tour, this_avoidance_map, this_sampling_reward] = knapsack_sampling_GPR(vine_distance, row_distance, reward_map, beginning, ending, budget, 0, sampling_map);
    elseif type == 3
    ...
    elseif type == 4
        [this_total_cost, this_total_reward, this_tour, this_avoidance_map, this_sampling_reward] = knapsack_sampling_GPR_extended(vine_distance, row_distance, reward_map, beginning, ending, budget, 0, sampling_map);
    end
    if this_sampling_reward >= min_samples
    ...
    while ((sampling_reward ~= this_sampling_reward) || (total_reward ~= this_total_reward)) && (abs(high_alpha - low_alpha) >= 10^-4)
        ...
        if (this_sampling_reward >= min_samples) && (sum(sum(reward_map)) ~= this_total_reward)
            total_cost = this_total_cost;
            total_reward = this_total_reward;
            tour = this_tour;
            avoidance_map = this_avoidance_map;
        ...
        elseif (this_sampling_reward >= min_samples)
            total_cost = this_total_cost;
            total_reward = this_total_reward;
            tour = this_tour;
            avoidance_map = this_avoidance_map;
