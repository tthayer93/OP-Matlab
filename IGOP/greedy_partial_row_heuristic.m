function [ total_cost, total_reward, tour ] = greedy_partial_row_heuristic( vine_distance, row_distance, reward_map, beginning, ending, budget )
%GREEDY_PARTIAL_ROW_HEURISTIC Builds an orienteering tour on an IG
%
%	Version: 1.0
%	Date: 01/24/2019
...
%		tour: A sequence of vertices describing the tour computed by GPR, from beginning vertex to ending vertex
    ...
    while sum(sum(cumulative_reward_1)) > 0 || sum(sum(cumulative_reward_2)) > 0
        ...
        if current_side == 1
            ...
            %% Check for best (partial) row and add to tour
